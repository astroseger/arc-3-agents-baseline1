...
def _level_step_count(attempts: list[AttemptInfo]) -> int:
    first_attempt = attempts[0]
    latest_attempt = attempts[-1]
    first_metadata = _read_json(first_attempt.path / "initial_metadata.json")
    latest_metadata = _read_json(_latest_metadata_path(latest_attempt.path, latest_attempt.n_steps))
...
def _read_attempt_info(attempt_dir: Path, level_index: int, attempt_index: int) -> AttemptInfo:
    ...
    step_numbers = _step_metadata_numbers(attempt_dir)
...
def inspect_sessions(session_dir: str | Path = DEFAULT_SESSION_DIR) -> SessionInspection:
    ...
    n_steps_by_level = {
        level_index: _level_step_count(attempts)
        for level_index, attempts in attempts_by_level.items()
    }
    n_steps_total = sum(n_steps_by_level.values())
    n_steps_current_level = n_steps_by_level[current_level_index]
