...
class CompetitionController:
    def __init__(self) -> None:
        self._remaining_game_id_mapping = dict(GAME_ID_MAPPING)
    ...
    def _start_session(
        ...
        real_game_id = self._remaining_game_id_mapping.pop(requested_game_id, None)
        if real_game_id is None:
            LOGGER.warning("rejecting start request for unknown or used game_id=%s", requested_game_id)
            return {"ok": False, "status": 400, "error": UNKNOWN_OR_USED_GAME_ID_ERROR}

        seed = int(payload.get("seed", 0))
        LOGGER.info("starting competition requested_game_id=%s seed=%s scorecard=%s", requested_game_id, seed, scorecard_id)
        env = arc.make(real_game_id, seed=seed, scorecard_id=scorecard_id)
        if env is None or env.observation_space is None:
            ...
            return {"ok": False, "status": 500, "error": f"failed to create environment for {requested_game_id}"}
