...
def start_game(
    ...
    rf.log_info(
        "starting game %s as alias %s with API key; container: %s",
        real_game,
        game_alias,
        container_name,
    )
    rf.LOGGER.info(
        "agent command real_game=%s game_alias=%s container=%s contains CODEX_API_KEY; command not logged",
        real_game,
        game_alias,
        container_name,
    )
...
def run_scheduler(
    ...
    try:
        ...
        while not stopping:
            ...
            if not running:
                for game_alias, real_game in game_id_mapping.items():
                    running[real_game] = start_game(
                        game_alias,
                        real_game,
