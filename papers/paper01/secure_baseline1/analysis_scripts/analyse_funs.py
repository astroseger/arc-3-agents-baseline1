...
def is_zero_move_reset_attempt(
    initial_metadata: Path,
    step_metadata_files: list[Path],
) -> bool:
    if not initial_metadata.is_file() or step_metadata_files:
        return False

    metadata = read_json(initial_metadata)
    action_input = metadata.get("action_input")
    return isinstance(action_input, dict) and action_input.get("id") == "RESET"
...
def iter_metadata_files(session_dir: Path) -> list[Path]:
    ...
    for attempt_dir in sorted(
        ...
        step_metadata_files = sorted(attempt_dir.glob("step_*_metadata.json"))
        if is_zero_move_reset_attempt(initial_metadata, step_metadata_files):
            break
        if initial_metadata.is_file():
            metadata_files.append(initial_metadata)
        metadata_files.extend(step_metadata_files)
