...
CLIENT_ROOT = Path(os.environ.get("GAME_CLIENT_ROOT", Path(__file__).resolve().parent))
SESSION_DIR = CLIENT_ROOT / "session"
CLIENT_STATE_PATH = SESSION_DIR / ".client_session.json"
CLIENT_LOCK_PATH = CLIENT_ROOT / ".client.lock"
SERVER_URL = os.environ.get("GAME_SERVER_URL", "http://127.0.0.1:8879")
...
COLOR_LOOKUP = np.array(
    [hex_to_rgb(COLOR_MAP.get(index, "#000000FF")) for index in range(16)],
    dtype=np.uint8,
)
BLACK_RGB = np.array(hex_to_rgb("#000000FF"), dtype=np.uint8)


def frame_to_rgb_array(frame: np.ndarray, scale: int = DEFAULT_PNG_SCALE) -> np.ndarray:
    height, width = frame.shape
    values = frame.astype(np.int64, copy=False)

    rgb_array = np.empty((height, width, 3), dtype=np.uint8)
    rgb_array[...] = BLACK_RGB

    valid = (0 <= values) & (values < len(COLOR_LOOKUP))
    rgb_array[valid] = COLOR_LOOKUP[values[valid]]
...
def read_client_state() -> dict[str, Any]:
    ...
    if keys != CLIENT_STATE_KEYS:
        ...
        raise RuntimeError(f"Invalid .client_session.json ({'; '.join(details)}).")

    for key in ("current_level", "current_attempt", "current_attempt_step"):
        value = int(state[key])
        if value < 0:
            raise RuntimeError(f"Invalid .client_session.json: {key} must be non-negative.")
        state[key] = value
    if int(state["current_level"]) < 1 or int(state["current_attempt"]) < 1:
        raise RuntimeError("Invalid .client_session.json: current_level and current_attempt must be positive.")
    if not str(state["session_token"]).strip():
        raise RuntimeError("Invalid .client_session.json: session_token is required.")
    if not str(state["server_url"]).strip():
        raise RuntimeError("Invalid .client_session.json: server_url is required.")
    return state


def write_client_state(state: dict[str, Any]) -> None:
    if set(state) != CLIENT_STATE_KEYS:
        raise RuntimeError("Refusing to write .client_session.json with unexpected keys.")
...
def session_token_from_state(state: dict[str, Any], *, allow_missing: bool = False) -> str | None:
    ...
    if token is None or str(token).strip() == "":
        if allow_missing:
            return None
        raise RuntimeError("No session_token in .client_session.json. Start a game first.")
...
def current_level_action_count(state: dict[str, Any]) -> int:
    level_id = int(state["current_level"])
    first_metadata_path = attempt_dir(level_id, 1) / "initial_metadata.json"
    last_metadata_path = metadata_path_for_state(state)
    if not first_metadata_path.is_file():
        raise FileNotFoundError(f"Missing metadata for first attempt of current level: {first_metadata_path}")
    if not last_metadata_path.is_file():
        raise FileNotFoundError(f"Missing metadata for current client state: {last_metadata_path}")

    first_metadata = json.loads(first_metadata_path.read_text(encoding="utf-8"))
    last_metadata = json.loads(last_metadata_path.read_text(encoding="utf-8"))
    return int(last_metadata["step_index"]) - int(first_metadata["step_index"])


def current_game_state(state: dict[str, Any]) -> str:
    metadata_path = metadata_path_for_state(state)
    if not metadata_path.is_file():
        raise FileNotFoundError(f"Missing metadata for current client state: {metadata_path}")
    metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
    game_state = str(metadata.get("state", "")).strip()
    if not game_state:
        raise RuntimeError(f"Missing game state in current metadata: {metadata_path}")
    return game_state


def validate_action_for_state(state: dict[str, Any], action: str) -> None:
    if action != "RESET" and current_game_state(state) == "GAME_OVER":
        raise ValueError("Only RESET is allowed when the game is in GAME_OVER state.")
    if action == "RESET" and int(state["current_attempt_step"]) == 0:
        raise ValueError("RESET is not allowed before taking any action in the current attempt.")
...
def move_command(action: str, x: int | None, y: int | None, reasoning_json: str | None) -> None:
    ...
    if x is not None or y is not None:
        ...
        if not (0 <= x <= 63 and 0 <= y <= 63):
            raise ValueError("ACTION6 coordinates must be inside the 64x64 frame: x and y must be in 0..63.")
    ...
    with client_lock():
        ...
        validate_action_for_state(state, canonical)
        level_action_count = current_level_action_count(state)
        if level_action_count >= MAX_LEVEL_ACTIONS:
            raise RuntimeError(
                f"Current level action limit reached: {level_action_count} actions on level "
                f"{state['current_level']} across all attempts (limit: {MAX_LEVEL_ACTIONS})."
            )
        response = post_json(
            "/game/action",
            payload,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="CLI client for the local server. One command per move."
    )
