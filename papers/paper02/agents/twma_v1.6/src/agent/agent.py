...
class Agent:
    def __init__(
        ...
        if mode not in {"master", "recovery"}:
            raise ValueError(f"Unsupported agent mode: {mode}")

        self.mode = mode
        self.run_dir = DEFAULT_AGENT_RUN_DIR
        if self.mode == "master":
            ensure_fresh_run_paths(self.run_dir)
            prepare_agent_run(self.run_dir)
        else:
            ensure_recovery_run_paths(self.run_dir)
        ...
        #     self.logger,
        #     [
        #         "world_model_simplification_step1.txt",
        #         "world_model_simplification_step2.txt",
        #         "world_model_simplification_step3.txt",
    ...
    def get_simple_continuation_prompt(self, inspection: SessionInspection) -> str:
    ...
    def send_simplification_prompts(self, inspection: SessionInspection) -> None:
        # is_early_first_attempt_level2 = (
        #     inspection.current_level_index == 2
        #     and inspection.attempts_per_level.get(2) == 1
        #     and inspection.n_steps_current_attempt < 10
        # )
        # if inspection.current_level_index == 1 or is_early_first_attempt_level2:
    ...
    def normal_continuation_protocol(self, inspection: SessionInspection) -> None:
        ...
        self.send_prompt(self.get_simple_continuation_prompt(inspection))

    def normal_reset_protocol(self, inspection: SessionInspection) -> None:
        self.logger.info("selected protocol", extra={"protocol": "normal_reset_protocol"})
        reset_string = self.run_client("move RESET")
        continuation_prompt = self.get_simple_continuation_prompt(inspection)
        prompt = (
            continuation_prompt
    ...
    def trouble_protocol1(self, inspection: SessionInspection) -> None:
        ...
        self.send_prompt(prompt_prefix)
        reset_string = self.run_client("move RESET")
        prompt = (
            self.continuation_string
            + "\n\nThe level has been reset. You have another attempt. Output from the client:\n"
    ...
    def reset_protocol(self, inspection: SessionInspection) -> None:
        ...
        if steps > self.last_trouble1[level] + 50:
            self.last_trouble1[level] = steps
            self.trouble_protocol1(inspection)
        else:
            self.normal_reset_protocol(inspection)
    ...
    def stuck_protocol(self, inspection: SessionInspection) -> None:
        ...
        # self.send_simplification_prompts(inspection)
        prompt = self.get_simple_continuation_prompt(inspection) + "\n" + self.stuck_reminder_prompt
        self.send_prompt(prompt)
...
def main(argv: list[str] | None = None) -> int:
    ...
    agent = Agent(
        mode="recovery" if args.recovery else "master",
