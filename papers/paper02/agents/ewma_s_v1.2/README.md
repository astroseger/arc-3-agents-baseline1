...
This directory contains the v1.2 flexible-interface executable-world-model agent with simplification and the Docker automation used to run it. See [AGENT.md](AGENT.md) for its implementation and [the parent README](../README.md) for the complete agent family and analysis utilities.

## Variant

- Codex must maintain textual and executable world models plus planning code, with interfaces of its own choosing.
- The controller runs the scheduled light or four-stage simplification procedure whenever Codex returns control.
- No fixed model templates or exact replay verifier utilities are supplied. The Docker image installs Codex CLI `0.128.0`.
