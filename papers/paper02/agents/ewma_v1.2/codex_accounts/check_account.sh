...
if login_status=$(docker run --rm -v "$codex_home_abs:/home/user/.codex" codex-agent-sequence-no-exactly-v-1-2 codex login status 2>&1); then
  :
else
  status=$?
...
if exec_output=$(docker run --rm -v "$codex_home_abs:/home/user/.codex" codex-agent-sequence-no-exactly-v-1-2 codex -m gpt-5.5 exec --skip-git-repo-check "Respond with exactly: PING" 2>&1); then
  :
else
  status=$?
