...
if login_status=$(docker run --rm -v "$codex_home_abs:/home/user/.codex" codex-agent-sequence-exactly-ws-v-1-5 codex login status 2>&1); then
  :
else
  status=$?
...
if exec_output=$(docker run --rm -v "$codex_home_abs:/home/user/.codex" codex-agent-sequence-exactly-ws-v-1-5 codex -m gpt-5.5 exec --skip-git-repo-check "Respond with exactly: PING" 2>&1); then
  :
else
  status=$?
